...
class Cc100(Dataset):
	...
	def __init__(self):
		...
		super().__init__('cc100')


	def import_dataset(self) -> None:

		self.dataset = load_dataset('oscar', 'unshuffled_deduplicated_fr', num_proc = NUM_THREADS)
	...
	def save(self, tokenizer: Tokenizer) -> None:
		...
		for split, documents in tokenized.items():
			...
			for batch_i in tqdm(range(SAVING_BATCHS), desc = f'Saving {self.name} {split}'):
